...
- **pgAdmin** trỏ `localhost:5433` chỉ hoạt động khi bạn chạy **`backend/educycle-java/docker-compose.yml`** (hoặc tự thêm `ports`).
- **`npm run dev` + chỉ chạy compose gốc** → proxy Vite **không** có API trên `localhost:8080` — cần thêm `mvn` local **hoặc** publish cổng `api`.

---

## 3. Backend (`backend/educycle-java`)
...
- Phiên bản trong repo tới **V15** (kiểm tra `backend/educycle-java/src/main/resources/db/migration/`); file mới = **V16** trở đi (không sửa migration đã apply).
...
- Định nghĩa trong [`src/App.jsx`](frontend/src/App.jsx): `Layout` + `lazy` pages + `ProtectedRoute` / `GuestRoute` / `adminOnly`.
...
- **Lỗi:** `getApiErrorMessage` / `userFacingMessage` trên interceptor ([`apiError.js`](frontend/src/utils/apiError.js)).
...
- CSS: token từ [`tokens.css`](frontend/src/styles/tokens.css) (không hardcode hex/pixel tùy tiện).
...
2. Postgres dev:
   ```bash
   cd backend/educycle-java && docker compose up -d
   ```
...
4. Frontend:
   - Tạo `frontend/.env.local` (hoặc chỉnh `.env.development`):
...
|---|----------------|------------|----------------|
| 1 | Cart/Wishlist “khóa học”, CartContext, USD… | ✅ | `CartContext` không còn trong `src/` (chỉ có thể sót trong skill/docs cũ). `CartPage` / `WishlistPage` theo hướng P2P. Nên quét copy trong `frontend` nếu vẫn thấy từ khóa lạ. |
