...
| 10 | CI không E2E | ✅ / ⚠️ | Có job **`e2e-api`** (Postgres + jar Spring + Playwright `request` tới `/actuator/health`, `/api/products`, 401 `/api/transactions`). Job **`e2e`** (Vite + smoke `document.title`) vẫn **không** golden path đăng nhập → OTP. |
...
| OAuth Google implicit | ⚠️ Mặc định **auth-code** + BE đổi code; `VITE_GOOGLE_OAUTH_USE_AUTH_CODE=false` → implicit. |
...
1. **E2E golden path** (login → đăng tin → giao dịch → OTP) trong CI — bổ sung cho job `e2e` hoặc mở rộng `e2e-api`.  
